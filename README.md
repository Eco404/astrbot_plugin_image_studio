...
当前开发版本为 **1.3.0-dev.2**，基于正式版 **1.2.0** 继续开发。新增 NovelAI 官方接口和 ComfyUI 工作流接入；ComfyUI 已完成真实服务器上的基础文生图、图生图与多输出测试，NovelAI 官方仍待可用账户完成生图验证。原有 NAI 第三方接口继续可用。完整变化见 [更新记录](CHANGELOG.md)。
...
本开发版另增加 ComfyUI 任务存储，数据库进入 `3-dev.1`。升级前同样自动备份数据库；如需退回正式版，应恢复升级前的完整数据备份，不能直接使用开发版数据库降级运行。
...
导入图片或 JSON 后，如果发现种子已固定，会显示节点、字段和值，并提示如何恢复随机。提示不会自动修改种子；标准采样节点应使用「种子」输入来源并设为 `-1`，支持负值随机模式的 rgthree Seed 节点也可直接将固定输入改为 `-1`。
