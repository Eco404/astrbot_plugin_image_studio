...
手机端页面标题随上滑逐步接替 Image Studio 导航卡，导航卡的上移、模糊与淡出由滚动位置直接控制，反向滚动同步恢复；画廊多选栏仍可进一步接替页面标题。图标按钮保持正圆，详情底栏各操作等距排列。

画廊分页以 24 条为基准，随实际列数补齐整行，最后一页保留真实剩余数量。分页、设置保存和详情操作栏始终位于对应页面或浮窗底部。多选操作栏从筛选区下方随页面上移，接近顶部时逐步接替画廊标题并吸顶；标题的上移、模糊和淡出直接由滚动位置决定，反向滚动时同步恢复。手机端使用紧凑图标按钮。设置存在修改时显示黄色未保存状态，切换页面保留草稿，保存失败不会清空修改。
...
图组按选择顺序排列，以第一张为封面。每张图片独立保留元数据和手动参数，详情翻页、参数复制、复现和导出使用当前图片的信息；收藏保护整个图组，也可删除其中指定图片。不同图组可以复用同一个原图文件，而不共享人工参数修改。分别导入仍保持原来的一图一条记录行为。
...
当前数据库结构为 `1-dev.2`，支持从 `1-dev.1` 保留数据升级，为图片关联增加独立补充参数。`PRAGMA user_version` 记录最近的正式基线（当前为 0），`schema_meta` 记录开发目标 1 和开发修订 2。配置文件版本、数据库版本、元数据解析器版本独立管理。
