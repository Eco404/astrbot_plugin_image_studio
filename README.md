...
- **导入**：点击拖拽区或拖入多张图片，自动读取生成参数，每张图片可独立补充来源、模型和参数，确认后上传入库。选择图片后拖拽区保持为紧凑的“继续添加图片”入口，也支持键盘操作。
- **设置**：管理浏览器主题、历史策略、Provider 并发、服务商、模型能力和 LLM 工具参数。

下拉选择控件采用与页面一致的玻璃样式，支持键盘选择和手机端操作。普通操作按钮边缘带轻微阴影，保留危险操作与键盘焦点的独立提示；导航项不加阴影，所有分段选择器仅为选中项显示框线。详情字段旁的复制按钮只显示图标，保留点击区域与键盘焦点提示，不显示圆形底色或阴影。画廊收藏筛选使用圆形星标按钮，点击切换“仅已收藏 / 全部”；刷新也使用圆形图标按钮。画廊刷新复用未变化的卡片与图片；详情参数在桌面端按两列独立排列，手机端显示为单列。

生图页选中 NAI 第三方服务商的模型时，标题栏右侧显示该服务商的剩余额度和停用状态；手机端优先显示额度。额度按服务商共享 30 秒短时缓存，只在生图页可见时定期查询，生成请求结束后失效并重新读取。查询通过所配置服务商的 `POST /api/api/getUser` 进行，不触发生图、不写入配置或历史。失败时显示“额度暂不可用”，不将错误显示为零额度，也不阻止生图；切换到其他类型模型后隐藏额度。

生成详情的主图下方显示本组图片的缩略图条，单张图片时隐藏。点击缩略图同步切换主图与参数，支持横向滚动和键盘方向键、Home、End；当前项突出显示并自动保持可见。缩略图条复用画廊预览，不等待原图加载，仅主图翻页或滑动可以跨到相邻记录。详情按打开时的画廊筛选顺序连续浏览，跨图组和跨画廊分页时保留同一个图片框；先准备相邻图片的预览，切换后同步图组信息与本组缩略图条。邻组只预取摘要和相邻缩略图，最多缓存三个图组摘要，不预取邻组原图；失败时保留当前图片，可重新尝试。

手机端同组和跨组横向拖动时，仅前景图片跟随手指，浮窗与模糊背景保持原位；松手后完成切换或回弹，实际切图后再更新参数、缩略图选中状态并交叉淡化背景。切换落位后，入场图片层会保留到实际主图节点完成解码与绘制交接，避免在 Safari 中短暂露出上一张图片。详情的模糊背景固定使用当前图片的缩略图，原图加载只升级前景，不重建背景或再次启动渐变。背景在下一张缩略图解码完成后才切换，加载失败时保留旧背景；原图准备好前保留预览，避免闪空。拖动期间不替换背景，开启系统“减少动态效果”时保留更短的纯透明度渐变。

图生图参考图使用页面按钮选择，显示当前数量与模型上限。超出剩余名额的文件不会上传；满额后禁用选择，删除参考图后可继续添加。

复选框和单选框的选中色、下拉选项的勾选标记，以及画廊、图组选择和详情缩略图的选中边框，都直接使用主题原色。勾选底色上的前景标志保留可读性；删除操作和收藏标记继续使用各自的语义颜色，开关开启时的键帽保持白色。

手机端详情切图同时判断拖动距离与最近的横向速度，支持短距离快速横扫；停手、反向拖回、轻微抖动和纵向滚动不会被当作快速切图。灯箱入场淡入不阻塞首次触摸；图片加载、解码和清晰度升级在手势及归位动画空闲后处理，连续切图跳过已经离开的原图。灯箱内不反复渲染下层详情，关闭或纵向退出前再同步最终图片。模糊背景使用常驻独立滤镜层，仅对子图片进行透明度渐变。这些优化减少操作期间的额外工作，不保证突破 iOS Safari 的动画刷新率策略。
...
主题设置采用 Quiet Glass 风格，可选择跟随系统、浅色或深色，调整强调色、饱和度（0–100%）和玻璃不透明度（20–100%），并恢复默认。除预设色外，还可以使用颜色选择器、输入 HEX 色值或吸色。手机端隐藏吸色按钮，保留颜色选择器和 HEX 输入。桌面端支持安全上下文中的浏览器 `EyeDropper` 时直接从屏幕吸色；不支持或被浏览器禁止时，可选择本地图片，在图片中点击或用方向键定位取色，确认后应用。图片只在浏览器内解码，不会上传或进入画廊；透明区域按白色背景取色。

按钮、开关与滑块直接使用选定的主题原色，正常和悬停时均不加深或调浅填充，只为其文字和图标选择可读的前景色。文字、焦点使用的强调色仍按明暗模式校正对比度，语义状态色不随强调色变化。页面标题、分页、多选、导入和设置悬浮栏，以及生成详情底栏与预览操作栏，都跟随主题玻璃效果；生成详情底栏共用浮窗的模糊层，通过少量补色将有效不透明度提高 6 个百分点，最高 100%，不再额外叠加一整层玻璃。系统要求增强对比或减少透明度时使用实色兜底。主题立即生效并静默保存，正常保存不显示提示或占位，只在读取、保存等操作失败时显示错误；不触发插件设置的未保存状态。偏好仅保存在当前浏览器，不写入插件配置、数据库或服务端文件。正常页面使用 `localStorage`；AstrBot 沙盒页面禁止访问该存储时，通过页面桥接保存为当前站点的浏览器 Cookie。清除浏览器站点数据会重置主题，不同浏览器之间不会同步。
