...
随机提示词无法静态还原时，插件会查找读取主管线同一文本输出端口的显示节点。目前支持 `easy showAnything`，关联快照会列入候选并标明执行图或界面工作流来源；内容一致时合并，存在冲突时由你选择。快照可能来自此前执行，不会自动填写为准确的生成参数，也不会重新执行随机抽卡。改用同一输出的另一份快照时，只替换此前采用且未被手动改写的文本。

同一工作流批量导入时，可以先在「最终保存输出」右侧批量应用分支，再用候选提示词按钮右侧的批量图标同步节点选择。每张匹配图片读取自己的参数与文本；已填入内容不重复追加，节点不匹配、快照来源有歧义或已被手动改写的片段会跳过，并提供逐文件结果。本次批量选择只作用于点击时已有的其他待导入图片，不影响之后加入的图片，也不会自动执行上传。
