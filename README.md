...
当前正式版本为 **1.3.0**。新增 NovelAI 官方接口、ComfyUI 工作流接入，以及供 Agent 使用的模型和工作流搜索。原有 NAI 第三方接口继续可用。完整变化见 [更新记录](CHANGELOG.md)。
...
从旧版升级前，请停止 AstrBot 并备份完整插件数据目录。1.3.0 使用正式 **v3 数据库**，增加 ComfyUI 工作流修订和任务记录：1.1.0／1.2.0 用户由 v2 升级，1.0.0 用户按 v1 → v2 → v3 升级；当前 `3-dev.1` 开发库可直接转换，保留已有工作流和任务数据。
