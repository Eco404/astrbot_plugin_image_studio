...
AstrBot 可能在附图旁显示 `[Image from tool ..., path='...']`；其中 `path` 是此次附图的临时文件路径，附图可能是预览图，也可能是原图。引用本插件图片时使用 `asset_id`；交付原图时调用 `image_studio_send_output`，将 `destination` 设置为 `session`。如需交给其他工具处理，将 `destination` 设置为 `workspace` 复制原图，再使用返回的文件路径。
