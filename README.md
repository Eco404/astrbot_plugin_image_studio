...
当前正式版本为 **1.4.2**。安装包见 [Releases](https://github.com/Eco404/astrbot_plugin_image_studio/releases)，完整变化见 [更新记录](CHANGELOG.md)。
...
批量查询最多 20 项，有效模型放在 `models`，无法使用的 ID 及原因放在 `errors`；相同模型只返回一份模型说明，`input_indices` 和 `requested_refs` 保留成功输入的对应关系，`input_index` 从 0 开始并指向首次输入。格式错误会拒绝整次查询，ID 错误不会影响其他有效项。生图工具仍使用单个字符串 `model_ref` 指定目标模型。

在处理当前这条用户消息期间，已查询的模型和生成模式可以多次使用同一份参数说明，修改提示词、种子或张数不需要重新查询。处理新的用户消息、使用尚未查询的模型或模式，或插件设置发生变化后，需要重新查询；查看、发送或复制已有图片无需查询模型。用户未指定模型时，设置 `query_type="default"` 查询对应模式的默认模型，只有默认模型不支持所需功能时才选择替代。风格描述本身不会要求换模型，`query_type="all"` 用于需要比较全部模型时。

模型选择与入参规则写在各工具描述中，插件不向系统提示词追加工作流约定。工具返回按当前结果给出下一步：服务商目录引导搜索模型、模型摘要引导查询可用模式及参数；任务、图片和投递结果分别说明自身状态。

当前这轮对话首次成功取得生图结果的 `asset_id` 后，若 `image_studio_send_output` 可用，插件会在本请求中隐藏 `send_message_to_user` 和 `pc_send_current_media`，统一由 Image Studio 交付生成的原图。通过任务查询取得结果或取得部分成功结果也适用；查询模型、搜索、查看旧图、生成失败及 ComfyUI 仍在等待时不会触发。触发后本轮后续失败不会恢复这两个入口，下条用户消息重新使用正常工具列表；不修改宿主或其他插件的全局工具配置。

ComfyUI 生图提交后可能返回 `pending` 和 `task_id`，此时使用 `image_studio_task` 查询同一任务，不再调用生成工具。查询可等待最多 30 秒，也会为 AstrBot 的工具调用超时预留余量；不填任务编号可找回当前会话最近任务。生成工具的可选 `request_id` 仅适用于 ComfyUI：同一会话中，相同标识和参数返回原任务，变更参数会报错；需要新图时使用新标识或省略。恢复只针对已有任务，不会重复提交 ComfyUI 已接收的生成任务。

图片已生成但未能返回原图的 `asset_id` 时，可使用任务工具和本次返回的 `task_id` 重试取得已有图片；非 ComfyUI 的 `result_` 编号仅在处理当前这条用户消息期间有效。复制到工作区部分失败时，成功项路径仍会返回，只需处理失败项，不应重新生图。生成结果标记为工具生成图片，查看资产则标记为已有图片；这些操作都不表示整个任务已经完成。

插件会为本轮返回的图片补上紧邻图片的来源说明：生成结果明确写为“由你调用生图工具生成”，查看已有图片则说明本次只查看、未生成；两者都注明本次工具调用未向用户发送图片。同时移除 AstrBot 为这些图片追加的原生发送工具建议，避免与插件的原图交付方式冲突。仅处理本轮 Image Studio 的工具结果，用户上传图片、其他工具和旧会话内容保持原样；宿主版本或消息结构无法识别时保留原行为。

AstrBot 附图旁的 `[Image from tool ..., path='...']` 标记仍保留；其中 `path` 是此次附图的临时文件路径，附图可能是预览图，也可能是原图。引用本插件图片时使用 `asset_id`；交付原图时调用 `image_studio_send_output`，将 `destination` 设置为 `session`。如需交给其他工具处理，将 `destination` 设置为 `workspace` 复制原图，再使用返回的文件路径。

发送工具中的 `plain` 只用于随产物发送的必要附言，普通最终正文由 Agent 直接回复，不放进 `messages`。会话投递成功后，结果会按是否包含文字提醒避免重复附言或重复发送图片；复制到工作区与投递未确认不会被报告为已发送给用户。这是发送指引，不会按内容自动拦截重复消息。

1.4.2 已在 AstrBot 4.27.4 的普通工具模式下完成真实生图与 QQ 私聊投递验证，并直接核对逐次模型请求中的工具列表和图片说明。使用 `skills_like` 两阶段工具模式且其他插件再次复制工具集合时，执行层屏蔽仍有已知限制；具体覆盖范围见 [测试说明](docs/TESTING.md)。
...
也可以带上图片或引用含图消息，请 Agent 修改画面。常规需求优先查询默认模型；如希望使用其他模型，可明确说明其名称，例如 NAI 或 NovelAI。
...
**机器人能聊天，但不调用生图工具？** 先检查当前人格是否启用了五个 Image Studio 工具，再检查插件和模型的工具开关，以及「工具」默认模型是否已设置。
