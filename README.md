...
图生图由模型配置中的「支持图生图」开关控制。模型和工具的参考图数量最少为 **1**，工具上限不能超过模型上限；关闭图生图会保留数量配置。模型能力未知时，新建模型默认关闭图生图，确认支持后可手动开启。每个模型可以设置原生批次上限和模型并发上限，服务商总并发限制仍同时生效。
...
- 服务商、模式、调用来源和生图来源支持多选筛选，默认全选；下拉菜单中可一键全选或清空。清空任一项时不显示匹配图片。
...
在「设置」中开启「扫描 NAI 插件图库」并保存，已有历史图片会逐步出现在画廊中。扫描卡片会显示张数和状态，支持手动重扫；后台每 5 分钟检查新增或删除的文件。

图片仍保存在 NAI 插件目录，Image Studio 只登记参数并生成缩略图。外部图片的 NovelAI 来源角标带有细描边，悬停可查看来源；存储健康会单独显示外部原图及缩略图占用。NAI 插件必须已有开启历史保存后留下的文件，无需让它保持运行。

- 外部图片不占 Image Studio 自动生图配额。收藏只记录状态，不能阻止 NAI 插件删除原图。
- 删除时会提示包含外部资源；确认会删除来源目录中的原图和对应参数文件。
- 用作参考图时保存独立原图副本，之后来源原图被清理也不影响这份参考图。
- 关闭扫描会隐藏该来源并回收可重建缩略图，保留索引及收藏状态；不会删除来源文件或已独立保存的本地图片。
