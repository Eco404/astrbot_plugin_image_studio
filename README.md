...
备份时请停止 AstrBot，并保存整个 `data/plugin_data/astrbot_plugin_image_studio` 目录。存储健康页面会分别显示原图、缩略图、ComfyUI 缓存、数据库、备份和暂存文件的占用；手动检查与自动维护使用同一套清理规则。选择深度检查时，在空间充足、没有待处理任务且可回收空间较多的情况下，还会整理数据库文件。
