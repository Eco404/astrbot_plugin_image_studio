...
从旧版升级前，请停止 AstrBot 并备份完整插件数据目录。当前开发版包含存储优化，会将旧数据库迁移到 **4-dev.1**，保留图库、收藏、工作流、任务和人工编辑数据。支持从正式 v1／v2／v3 及已支持的最终开发库升级；迁移后的目录需配合当前版本使用。
...
- “画廊卡片信息显示”默认开启；关闭后隐藏卡片底部的模型、提示词、模式和时间，图片、来源角标及选择操作继续保留。该选项随「保存全部设置」保存到当前浏览器，未保存时离开设置页可选择放弃调整。
