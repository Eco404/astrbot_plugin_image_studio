...
批量查询最多 20 项，有效模型放在 `models`，无法使用的 ID 及原因放在 `errors`；相同模型共用一份契约，`input_indices` 和 `requested_refs` 保留成功输入的对应关系，`input_index` 从 0 开始并指向首次输入。格式错误会拒绝整次查询，ID 错误不会影响其他有效项。生图工具仍使用单个字符串 `model_ref` 指定目标模型。

同一轮对话中，只要配置未改变，已查询的完整能力可以重复使用；查看、发送或复制已有资产无需再查询模型。未指定模型时先查 `default`，优先使用对应模式默认模型，只有能力不满足时才选择替代。风格描述本身不会要求换模型，`all` 用于确实需要比较完整模型目录时。

ComfyUI 生图提交后可能返回 `pending` 和 `task_id`，此时使用 `image_studio_task` 查询同一任务，不再调用生成工具。查询可等待最多 30 秒，也会为宿主工具超时预留余量；不填任务编号可找回当前会话最近任务。生成工具的可选 `request_id` 仅适用于 ComfyUI：同一会话中，相同标识和参数复用同一个任务，变更参数会报错；需要新图时使用新标识或省略。恢复只针对已有任务，不会重提已确认的远端工作流。

图片已生成但原图资产登记失败时，返回恢复编号，可使用任务工具重新登记已有结果；非 ComfyUI 的 `result_` 编号仅在当前轮有效。复制到工作区部分失败时，成功项路径仍会返回，只需处理失败项，不应重新生图。生成结果标记为工具生成图片，查看资产则标记为已有图片；这些操作都不表示整个任务已经完成。
