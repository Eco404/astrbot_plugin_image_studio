...
任务恢复窗口默认为 7 天；图库保存成功后复用已有原图，过期任务归档必要摘要。运行中、状态未知及其依赖任务仍保留恢复资源，图库原图按原有规则管理。当前不处理视频、音频或任意界面图自动转换。
...
ComfyUI 批次共享相同图片和工作流快照，图库保存成功后复用已有原图。任务恢复窗口默认为 7 天，过期任务保留必要摘要；运行中和状态未知的任务及其依赖仍受到保护。已保存的图库图片和复现参数不随任务归档删除。升级数据库备份在维护时保留最近 3 份，并保护本次迁移备份；长期备份请另行保存。
