...
3. 选择结果节点，按需将文字、数值、枚举、参考图或蒙版绑定到具体节点输入。无需调整的内容保持工作流原值，纯固定工作流不必填写主提示词。
...
任务进入队列后，刷新页面仍可查询进度。网络中断时不要重新点击生成；已有远端编号的任务可使用“继续查询”恢复。没有收到远端编号的提交显示为状态未知，不会自动重提。取消只针对本插件的任务，旧版 ComfyUI 可能只能移除排队任务。

未绑定数量参数时，一次运行保留工作流全部指定输出。绑定数量后，超过原生批次的请求会按并发限制分批执行，最终合并为一个图组；有多个输出节点时，最终图片数可能多于请求数量。任务临时输入和结果在终止后保留 7 天，画廊中的原图仍按原有图库规则管理。运行中和状态未知的任务不按此期限清理。首版不处理视频、音频或任意界面图自动转换。
