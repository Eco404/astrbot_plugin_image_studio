...
从旧版升级前，请停止 AstrBot 并备份完整插件数据目录。当前开发版包含存储优化和图组标题，会将旧数据库迁移到 **4-dev.2**，保留图库、收藏、工作流、任务和人工编辑数据。支持从正式 v1／v2／v3 及已支持的最终开发库升级；4-dev.1 升级时仅新增空标题字段，不重复转换正文。迁移后的目录需配合当前版本使用。
...
导入卡片的「文本节点识别」和底部警告默认折叠；警告收起时显示「需注意 N」，展开可查看完整内容。对于未适配的文本节点，展开「文本节点识别」后点击节点条目，打开绑定窗口：
