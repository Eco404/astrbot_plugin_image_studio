...
图生图由模型配置中的「支持图生图」开关控制。模型和工具的参考图数量最少为 **1**，工具上限不能超过模型上限；关闭图生图会保留数量配置。能力未知的自定义模型新建时默认关闭图生图，确认支持后可手动开启。NovelAI 官方内置的 V4.5/V5 Full、Curated 模型均预填为支持基础单底图图生图，最多使用 1 张参考图。每个模型可以设置原生批次上限和模型并发上限，服务商总并发限制仍同时生效。
...
4. 四个内置模型默认开启图生图，当前只接收一张底图，并将其缩放到输出尺寸；重绘强度和新增噪声只在图生图面板显示。角色参考、Vibe Transfer 和局部重绘尚未接入。

标题栏会分别显示 Anlas 余额与 V5 使用额度，查询失败不影响生图。随机种子使用 `-1`，实际返回的种子会逐图保留；详情中的「实际请求」和复现使用当前图片的数据，仍遵循参数记录、显示与回填设置。PNG 透明通道中的 NovelAI 隐写参数也可解析，原图不会因此被修改。
