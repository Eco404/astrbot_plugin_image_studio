...
| 标识 | 当前值 | 含义 |
| --- | --- | --- |
| 插件版本 | `1.4.1-dev.1` | `metadata.yaml` 与 `main.py` 注册版本相同，正式基线为 `1.4.0` |
...
测试需覆盖新目录初始化、旧目录迁移、同内容目标复用、内容冲突、符号链接拒绝、事务回滚、中断后的重复启动，以及迁移后的图库原图、缩略图和 ComfyUI 结果读取。文件布局变化不由 `PRAGMA user_version` 表示，不单独增加数据库修订；当前数据库为 v4，插件为 `1.4.1-dev.1`。
...
1. 当前开发版 `1.4.1-dev.1` 沿用 1.4.0 的正式数据库 v4，包含共享正文存储和标题字段，已发布的 v1／v2／v3／v4 定义与转换契约需要保持不变。
...
当前开发版构建默认输出 `dist/astrbot_plugin_image_studio-v1.4.1-dev.1.zip`，实际文件名跟随元数据版本；支持 `--root` 和 `--output`。构建读取当前工作区，不要求先提交，不执行数据库转换。
