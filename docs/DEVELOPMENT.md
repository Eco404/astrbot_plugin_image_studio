...
当前正式版本为插件 `1.4.0`。存储优化及图组标题使用正式数据库 **v4**：`PRAGMA user_version=4`，不保留 `schema_meta` 开发标记。旧正式库通过备份保护的迁移升级，最终开发库 4-dev.2 核验后转换为正式库，不重复编码正文。ComfyUI 已完成基础真实生图测试；NovelAI 官方仍需可用账户完成成功生图验证。
...
| 标识 | 当前值 | 含义 |
| --- | --- | --- |
| 插件版本 | `1.4.0` | `metadata.yaml` 与 `main.py` 注册版本相同 |
...
历史版本对应关系：插件 1.0.0 使用数据库 v1，1.1.0 和 1.2.0 使用数据库 v2；1.3.0 开发后期使用 `3-dev.1`，后续正式版转换为 v3。1.3.2 及存储优化前的 `1.4.0-dev.1` 使用 v3；共享正文存储使用 4-dev.1，图组标题扩展为最终 4-dev.2，1.4.0 正式版使用 v4。插件版本、数据库版本、配置格式版本和解析器版本分别维护。
...
回退应同时恢复升级前代码与完整数据副本。不要让旧版插件打开正式 v4 库，也不要把自动备份覆盖到正在运行的数据库上；数据库备份不包含旧任务目录转为共享文件后的文件布局变化，回退尤其需要完整目录副本。不存在自动降级迁移。
...
测试需覆盖新目录初始化、旧目录迁移、同内容目标复用、内容冲突、符号链接拒绝、事务回滚、中断后的重复启动，以及迁移后的图库原图、缩略图和 ComfyUI 结果读取。文件布局变化不由 `PRAGMA user_version` 表示，不单独增加数据库修订；当前数据库为 v4，插件为 `1.4.0`。
...
1. 当前正式 `1.4.0` 使用数据库 v4，包含共享正文存储和标题字段，已发布的 v1／v2／v3／v4 定义与转换契约需要保持不变。
...
当前正式版构建默认输出 `dist/astrbot_plugin_image_studio-v1.4.0.zip`，实际文件名跟随元数据版本；支持 `--root` 和 `--output`。构建读取当前工作区，不要求先提交，不执行数据库转换。
