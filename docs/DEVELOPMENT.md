...
当前开发线为插件 `1.2.0-dev.1`，基于正式插件 `1.1.0` 和数据库 v2；数据库结构没有变化，开发库继续使用正式 `user_version=2`，无需新的迁移标记。

## 1.1.0 正式基线

本次正式插件版本为 `1.1.0`，图库数据库版本为 **v2**。开发期的外部来源修订已收拢为一次 `v1 → v2` 发布迁移；新库直接创建最终结构，正式库不保留 `schema_meta` 开发标记。
...
1. 从 `1.1.0` 和数据库正式 v2 基线继续开发，不修改已发布结构的版本含义。
...
4. 插件开发版本可使用 `1.2.0-dev.1` 等名称，与数据库版本独立。开发测试使用临时目录或独立数据副本，不与正式部署共用数据目录。
...
构建默认输出 `dist/astrbot_plugin_image_studio-v1.1.0.zip`，实际文件名跟随元数据版本；支持 `--root` 和 `--output`。构建读取当前工作区，不要求先提交，不执行数据库转换。
