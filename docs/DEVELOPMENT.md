...
历史版本对应关系：插件 1.0.0 使用数据库 v1，1.1.0 和 1.2.0 使用数据库 v2；1.3.0 开发后期使用 `3-dev.1`，1.3.1 正式版将其转换为 v3，1.3.2 及其开发版沿用该结构和转换路径，当前 `1.4.0-dev.1` 继续沿用。插件版本、数据库版本、配置格式版本和解析器版本分别维护。

## 升级与开发库转换

| 当前数据库 | 当前版本的处理（沿用 1.3.2） |
| --- | --- |
| 空库 | 直接创建正式 v3，不备份空库 |
| 正式 v1（1.0.0） | 核验、备份，在一个事务中执行 `v1 → v2 → v3` |
| 正式 v2（1.1.0／1.2.0） | 核验、备份，执行一次 `v2 → v3` |
| 正式 v3（含 1.3.2 开发版与 1.4.0-dev.1） | 只校验结构，不重复备份和迁移 |
