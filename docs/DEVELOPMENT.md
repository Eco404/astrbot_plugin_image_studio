...
当前开发版本为插件 `1.4.0-dev.1`，基于正式版 `1.3.2` 开发。存储优化及图组标题使用数据库 **4-dev.2**：`user_version` 保持上一正式版本 3，以 `schema_meta(4,2)` 明确标记当前开发结构。安装时备份并迁移现有数据库，不应使用旧版代码直接打开迁移后的库。ComfyUI 已完成基础真实生图测试；NovelAI 官方仍需可用账户完成成功生图验证。
...
| 数据库正式基线／当前目标 | `3` / `4-dev.2` | `PRAGMA user_version=3`，开发标记为 `schema_meta(target_version=4,dev_revision=2)` |
...
数据库结构和发布迁移集中在 `backend/database/schema.py`。已发布的 v1、v2、v3 定义与升级 SQL 保持不变。当前新库创建完整 4-dev.2，已有正式库先走保留的升级路径，再迁移共享正文、轻量任务/图库投影与引用清理结构，并添加空标题字段。已有 4-dev.1 仅执行标题字段迁移。正文编码及生命周期见 [存储与维护](STORAGE_LIFECYCLE.md)。
...
历史版本对应关系：插件 1.0.0 使用数据库 v1，1.1.0 和 1.2.0 使用数据库 v2；1.3.0 开发后期使用 `3-dev.1`，后续正式版转换为 v3。1.3.2 及存储优化前的 `1.4.0-dev.1` 使用 v3；共享正文存储使用 4-dev.1，图组标题扩展为 4-dev.2。插件版本、数据库版本、配置格式版本和解析器版本分别维护。
...
| --- | --- |
| 空库 | 创建 4-dev.2，不备份空库 |
| 正式 v1（1.0.0） | 核验、备份，在一个事务中执行 `v1 → v2 → v3 → 4-dev.1 → 4-dev.2` |
| 正式 v2（1.1.0／1.2.0） | 核验、备份，执行 `v2 → v3 → 4-dev.1 → 4-dev.2` |
| 正式 v3 | 核验、备份，转换正文并添加标题，迁移到 4-dev.2 |
| `user_version=3`、`schema_meta(4,1)` | 核验、备份，仅添加 `generations.title` 并更新开发标记，不重复转换正文 |
| `user_version=3`、`schema_meta(4,2)` | 只校验结构与引用清理触发器，不重复迁移 |
| `user_version=2`、`schema_meta(3,1)` | 核验最终 3-dev.1 布局、备份，转换并升级到 4-dev.2 |
| `user_version=1`、`schema_meta(2,2)` | 核验最终 2-dev.2 布局、备份，转换并升级到 4-dev.2 |
| `user_version=0`、`schema_meta(1,3)` | 核验最终 1-dev.3 布局、备份，转换并升级到 4-dev.2 |
...
备份失败不继续升级。迁移在 `BEGIN IMMEDIATE` 事务内再次核验数据库状态，正文迁移及引用校验成功后才写入开发标记；中途失败整体回滚，备份保留。常规维护独立轮换已验证的升级备份，保留最近 3 份并保护本次迁移备份；未知文件、损坏备份和符号链接不参与自动删除。

自动备份只包含数据库，**不包含配置和图片文件**。升级前应停止 AstrBot，备份完整插件数据目录；不要只复制运行中的主 `.sqlite3` 文件而忽略 WAL 数据。

回退应同时恢复升级前代码与完整数据副本。不要让旧版插件打开 4-dev.2 库，也不要把自动备份覆盖到正在运行的数据库上；数据库备份不包含旧任务目录转为共享文件后的文件布局变化，回退尤其需要完整目录副本。不存在自动降级迁移。
...
测试需覆盖新目录初始化、旧目录迁移、同内容目标复用、内容冲突、符号链接拒绝、事务回滚、中断后的重复启动，以及迁移后的图库原图、缩略图和 ComfyUI 结果读取。文件布局变化不由 `PRAGMA user_version` 表示，不单独增加数据库修订；当前数据库为 4-dev.2，插件为 `1.4.0-dev.1`。
...
1. 当前 `1.4.0-dev.1` 从正式 v3 升级至 4-dev.2，分别经过 4-dev.1 存储转换和 4-dev.2 标题字段扩展，不修改已发布结构的版本含义。
2. 普通 UI、指令或 Provider 修改不单独提高数据库修订；结构改变需要明确的新开发修订及受控迁移。
3. 开发库保留 `user_version=3`，以 `schema_meta(target_version=4,dev_revision=2)` 标识当前布局；结构、正文编码和开发标记在同一事务内提交。
...
当前开发版构建默认输出 `dist/astrbot_plugin_image_studio-v1.4.0-dev.1.zip`，实际文件名跟随元数据版本；支持 `--root` 和 `--output`。构建读取当前工作区，不要求先提交，不执行数据库转换。
