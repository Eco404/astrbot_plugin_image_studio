...
查看 LLM trace 时，应检查工具执行后下一次模型请求的 `messages`。AstrBot 的工具日志及中间 `tool_call_result` 事件在包装修正前产生，仍显示旧说明不等于模型收到旧说明。本次验证使用模拟结果，不调用真实 LLM、生图或发送服务；不能据此宣称模型已在真实会话中正确理解来源。此适配不修复其他插件再次复制工具集合的兼容性缺口，也不清理旧会话中的提示词。
