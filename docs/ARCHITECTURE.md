...
`main.py` 保留 AstrBot 的插件类、装饰器和注册签名。插件不注册用于追加系统提示词的请求钩子，也不修改 AstrBot 或其他插件的工具集合。调用前的模型选择、入参和资产使用约定写在各工具描述中，动态状态及下一步只由相应工具结果返回。能力工具调用 `query_capabilities(settings, ...)` 获取载荷，再由入口维护本次事件和配置修订的能力查询状态；返回说明明确限定当前用户消息、已查询的 `model_ref` 及 `query_modes`。修改生成参数值不要求重新查询，新事件或插件配置修订改变后则需重新查询。搜索摘要不会授予直接生成的权限。纯载荷模块不导入插件入口或 AstrBot 事件。
...
自有静态资源使用 `1.4.1-dev.1` 缓存标识，避免升级后旧脚本与新目录混用；插件基于正式版 `1.4.0` 继续开发。
...
图片布局迁移先预检冲突和符号链接，校验后通过链接或复制准备目标文件，再用单一事务更新资产、预览和 ComfyUI 已知图库路径，提交后清除已核验旧副本及空目录。迁移日志支持中断续跑，未知旧目录内容保留；日常统计兼容新旧布局。这一文件与路径转换不增加数据库修订；当前数据库为正式 v4，插件版本为 `1.4.1-dev.1`。
