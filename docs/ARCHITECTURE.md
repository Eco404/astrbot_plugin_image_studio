...
`main.py` 保留 AstrBot 的插件类、装饰器和注册签名。能力工具调用 `query_capabilities(settings, ...)` 获取载荷，再由入口维护本次事件和配置修订的能力查询状态；契约可重复使用，配置改变后需重新查询。搜索摘要不会触发完整能力授权，查询能力也不会改变发送工具集合。纯载荷模块不导入插件入口或 AstrBot 事件。

能力结果与执行共享 `ImageModel.parameter_default`，工具默认值依次读取 `default_override`、兼容字段 `default`、模型默认值。参数按已查询模式过滤，并保留适用模式、数值边界与可空标记；界面说明中的相同数值范围不重复发送。`default/all/model/search` 使用同一默认模型标记与选择顺序：用户明确指定、对应模式默认、明确能力缺口时的替代模型。`model_refs` 每次最多 20 项，重复模型只返回一份完整契约，`input_indices` / `requested_refs` 保留各成功输入的对应关系；错误仍按原输入索引返回。

ComfyUI 完整能力保留管理员提示词说明，用 `prompt_contract.required` 表示是否有主提示词入口，`reference_slots` 只公开配置名称、从 0 开始的参考图序号、图片/蒙版类型及必填状态，不公开节点图。内置使用说明不推翻用户默认选择；未知 NovelAI 型号不自动宣称支持自然语言。能力查询的 `asset_policy` 仅说明当前返回方式，资产处理规则由系统工具约定和实际结果集中说明。
