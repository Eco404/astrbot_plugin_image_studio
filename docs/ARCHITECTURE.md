...
宿主会在 `step` 结束前输出工具日志和中间 `tool_call_result` 事件，因此这些位置可能仍含原始包装文字。图片说明的验收位置是下一次模型请求的消息列表；原生工具事件不能单独证明模型收到的最终内容。1.4.2 已通过一次真实链路测试，直接确认后续模型请求收到替换后的图片说明与工具列表；此适配不改变多模态接口的消息角色，单次结果也不能保证所有模型始终正确理解来源。验证方式和已知的两阶段工具集合副本限制见 [测试说明](TESTING.md)。
