...
backend/
  ...
  tools/
    ...
    image_context.py            本轮工具附图来源说明与宿主包装适配
  generation/service.py         生图校验、批次、并发、结果登记与复现
  generation/concurrency.py     服务商、模型与官方账号共享限流
  generation/comfyui_runtime.py  ComfyUI 生图、图库、会话任务与恢复协调
  comfyui/
  ...
  providers/
    ...
    comfyui/
      ...
      job_store.py              任务/修订事务、会话查询、引用、归档及清理决策
      job_manager.py            异步任务生命周期、有界等待、恢复与关闭
...
`main.py` 保留 AstrBot 的插件类、装饰器和注册签名。插件不注入系统提示词；`on_llm_request` 只复制并记录本请求的工具集合，准备阶段保留所有工具名称。独立副本避免后续调整影响全局集合或下一条消息。调用前的模型选择、入参和资产使用约定写在各工具描述中，动态状态及下一步只由相应工具结果返回。能力工具调用 `query_capabilities(settings, ...)` 获取载荷，再由入口维护本次事件和配置修订的能力查询状态；返回说明明确限定当前用户消息、已查询的 `model_ref` 及 `query_modes`。修改生成参数值不要求重新查询，新事件或插件配置修订改变后则需重新查询。搜索摘要不会授予直接生成的权限。纯载荷模块不导入插件入口或 AstrBot 事件。

生图后的发送入口选择由插件入口按当前事件管理：首次成功登记并返回生成结果的 `asset_id` 后，确认本请求有 `image_studio_send_output`，再仅对本请求隐藏 `send_message_to_user` 和 `pc_send_current_media`。请求使用独立的 ToolSet，并记录通过其公开工厂方法创建的精简说明、参数说明副本；过滤同时覆盖这些副本及原始执行列表，避免 AstrBot 的 `skills_like` 模式再次展示被屏蔽工具的参数或执行它们。任务查询取得生成结果及部分成功结果走同一条件；能力查询、搜索、查看旧图、失败和 pending 不触发。状态不写入配置或数据库，后续失败不会解除已生效的本轮限制，下个事件使用新请求的正常工具列表；宿主或其他插件的全局工具定义保持原样。

最终正文与产物附言的约定仅放在 `image_studio_send_output` 的工具说明中：普通最终正文直接输出，`plain` 仅用于必要附言。发送结果只有在 session 投递成功后才根据有无文字给出避免重复的指引；workspace 结果只表示复制，未知投递状态不声称已发送。这不新增消息内容指纹、投递去重或全局对话约束。

能力结果与执行共享 `ImageModel.parameter_default`，工具默认值依次读取 `default_override`、兼容字段 `default`、模型默认值。参数按已查询模式过滤，并保留适用模式、数值边界与可空标记；界面说明中的相同数值范围不重复发送。`default/all/model/search` 保留同一默认模型标记，选择顺序由工具描述统一约定：用户明确指定、对应模式默认、明确能力缺口时的替代模型；各查询结果的 `next_action` 只指引当前查询的下一步，不重复整套规则。`model_refs` 每次最多 20 项，重复模型只返回一份完整契约，`input_indices` / `requested_refs` 保留各成功输入的对应关系；错误仍按原输入索引返回。

ComfyUI 的模型说明保留管理员提示词说明，用 `prompt_contract.required` 表示是否绑定生成工具的 `prompt` 参数，`reference_slots` 只公开配置名称、从 0 开始的参考图序号、图片/蒙版类型及必填状态，不公开节点图。内置使用说明不推翻用户默认选择；未知 NovelAI 型号不自动宣称支持自然语言，也不引导读取未返回的 `novelai_capabilities`。能力查询的 `asset_policy` 仅说明当前返回方式，资产处理规则由相应工具描述和实际结果说明；附图说明直接对应 AstrBot 显示的 `[Image from tool ..., path='...']` 标记。

生成结果和已有资产查看结果先返回来源说明，再附视觉内容，避免把框架包装的工具图片误认为用户上传。部分成功由执行结果明确标记，普通 warning 不改变成功状态；生成、画廊记录、资产登记和投递分别处理失败。普通服务商生图后的画廊/参考清理失败保留输出；资产登记失败的 `result_` 恢复编号只在当前事件有效，ComfyUI 则使用持久任务恢复。查看和投递仍校验资产所属会话，但不要求模型能力查询；批量复制先完成全量授权与格式校验，再逐项写入并报告成功路径和失败项。

`backend/tools/image_context.py` 在运行时包装 AstrBot 的 `ToolLoopAgentRunner.step`，不修改宿主文件。`on_agent_begin` 将状态绑定到当前插件实例、事件和 `run_context`；生成及查看入口登记本插件实际返回的 JSON 正文、图片索引及资产编号。原 `step` 正常耗尽后，在下一轮请求与上下文压缩前，只校验上下文末尾的本次工具调用组：工具名称、正文、调用编号、缓存图片路径、图片标记与相邻图片的 `image_url.id` 均须对应。全部验证通过后，原位删除工具正文中宿主追加的原生发送建议，并在图片标记旁补充生成结果或已有图片查看的说明与 `asset_id`。原消息和图片对象、`user` 角色、图片数据与宿主其余提醒均保留；不扫描旧消息或改写其他工具的内容。

安装要求 AstrBot 4.x 及已知的异步生成器签名；版本或签名不匹配则不安装。结果被截断、结构或图片对应关系不符时放弃本步改写、清除待处理状态并记录一次警告，不读取或修改宿主溢出文件。取消或异常不会应用未完成结果。重复安装不叠加有效处理；卸载时仅在类方法仍为自身包装的情况下恢复原方法，若其他插件已加外层包装则只停用自身层，保留外层包装。这一适配不扩大工具集合过滤范围，也不改变旧会话。

宿主会在 `step` 结束前输出工具日志和中间 `tool_call_result` 事件，因此这些位置可能仍含原始包装文字。图片说明的验收位置是下一次模型请求的消息列表；原生工具事件不能单独证明模型收到的最终内容。此适配不改变多模态接口的消息角色，也不代表已通过真实 LLM 的来源理解验证。
...
对话中的 ComfyUI 请求由入口调用 `ComfyRuntime.submit_agent()`，并通过 `image_studio_task` 接回 `inspect_agent()` 或 `list_agent()`；WebUI、指令和对话共用原任务管理器。提交与查询返回内部任务对象，入口只向模型暴露状态、进度、恢复信息和最终图片资产；最近任务列表直接使用轻量投影。`job_manager.wait(..., timeout_seconds=...)` 到期返回当前状态，调用方取消等待也不取消工作流。恢复仅调度同一持久任务，已有远端请求不会因工具超时而重复提交。
...
自有静态资源使用与插件正式版一致的 `1.4.2` 缓存标识，避免升级后旧脚本与新资源混用；第三方库保留自身版本号。
...
图片布局迁移先预检冲突和符号链接，校验后通过链接或复制准备目标文件，再用单一事务更新资产、预览和 ComfyUI 已知图库路径，提交后清除已核验旧副本及空目录。迁移日志支持中断续跑，未知旧目录内容保留；日常统计兼容新旧布局。这一文件与路径转换不增加数据库修订；当前数据库为正式 v4，插件版本为 `1.4.2`。
...
对话任务授权使用 `comfy_jobs.request_json.agent.scope_hash`，与可选的调用身份展示字段分开；归档仍保留该哈希和请求标识。仓储在展开正文前按会话范围过滤，无绑定的旧任务不会被自动授予对话权限。显式 `request_id` 与会话哈希共同派生任务 ID，复用已有 `BEGIN IMMEDIATE` 事务和请求指纹校验实现幂等；未提供标识的请求仍独立执行。新增任务查询和恢复接口只使用现有 JSON 字段，不改变正式数据库 v4、不触发结构迁移。详细行为见 [ComfyUI 接入](COMFYUI_IMPLEMENTATION.md)。

1.3.x 的数据库正式基线为 v3，已发布迁移保持不变。1.4.0 使用正式 v4：任务、图库和元数据仓储通过 `database/payloads.py` 共享不可变正文，各自持有引用；`gallery/storage.py` 维护轻量投影与按需展开，`providers/comfyui/storage.py` 管理任务编码。正式 `v3 → v4` 的结构、正文和版本在同一备份保护的事务内提交；细节见 [开发与发布维护](DEVELOPMENT.md)及[存储生命周期](STORAGE_LIFECYCLE.md)。

`database/migrations/v4_storage.py` 沿用开发阶段冻结的正文转换契约，不调用日常业务仓储、当前解析器或可变投影代码。版本转换中相似的编码代码属于有意冻结的历史契约，不能随当前业务重构一起改写。正式 v4 将存储结构和图组标题纳入同一发布迁移；最终 4-dev.2 核验及备份后只转换版本标记，不重复编码正文。
