(function () {
  ...
  window.ImageStudioSettings = function (hooks) {
    ...
    function ensureToolConfig(model, provider) {
      ...
      const defaults = { enabled: true, selection_description: nai ? "使用英文标签生成插画。" : "适合一般自然语言生图需求。", prompt_profile: nai ? "nai_tags" : "natural_language", prompt_instructions: nai ? "使用英文逗号分隔标签，保留用户指定的主体、数量、动作、服装与构图。" : "使用清晰、连贯的自然语言描述，不要使用英文逗号分隔的 NAI tag 串。", negative_prompt_exposed: !!model.supports_negative_prompt, max_reference_images: model.max_reference_images, parameters: {} };
      if (official) Object.assign(defaults, { selection_description: novelaiModels.some(item => item.id === model.id) ? "NovelAI 官方图像生成。参考图片支持哪些用途，请查看查询结果中的 novelai_capabilities。" : "NovelAI 官方图像生成。此模型的参考图片用途尚未核实。", prompt_profile: "custom", prompt_instructions: novelaiModels.some(item => item.id === model.id) ? "使用英文逗号分隔标签，可结合自然语言补充关系与动作；保留用户明确指定的内容。" : "优先使用英文逗号分隔标签；其他提示词格式请按所用模型的说明配置。" });
