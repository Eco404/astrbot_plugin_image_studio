(function () {
  ...
  async function bridge() { return window.AstrBotPluginPage.ready(); }
  ...
  function renderProviderEditor() {
    ...
    els.providerForm.innerHTML = `<h3>${escape(provider.name || "Provider")}</h3>${field("id", "ID", provider.id)}${field("name", "名称", provider.name)}${selectField("kind", "类型", provider.kind, kinds)}${field("base_url", "Base URL", provider.base_url)}${field("model", "默认模型", provider.model)}${field("generate_path", "文生图路径", provider.generate_path)}${field("edit_path", "图生图路径", provider.edit_path)}${field("api_key", "API Key", provider.api_key)}${field("timeout_seconds", "超时秒数", provider.timeout_seconds, "number")}${selectField("edit_request_format", "图生图格式", provider.edit_request_format, [["multipart", "multipart"], ["json_data_url", "JSON data URL"]])}<div class="toggle-row"><label>启用</label><input data-provider-field="enabled" type="checkbox" ${provider.enabled ? "checked" : ""}></div><div class="toggle-row"><label>支持文生图</label><input data-provider-field="supports_text2img" type="checkbox" ${provider.supports_text2img ? "checked" : ""}></div><div class="toggle-row"><label>支持图生图</label><input data-provider-field="supports_img2img" type="checkbox" ${provider.supports_img2img ? "checked" : ""}></div>${field("max_reference_images", "最大参考图数", provider.max_reference_images, "number")}${textAreaField("custom_headers", "自定义请求头", provider.custom_headers)}${textAreaField("request_template", "自定义 JSON 请求模板", provider.request_template)}${field("response_image_path", "响应图片路径", provider.response_image_path)}<div class="provider-editor-actions"><button class="danger-button" id="removeProviderButton" type="button">删除 Provider</button><button class="quiet-button" id="testProviderButton" type="button">测试 Provider</button></div>`;
