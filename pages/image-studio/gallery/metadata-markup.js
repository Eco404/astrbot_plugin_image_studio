(function () {
  ...
  window.ImageStudioMetadataMarkup = function ({ escape, parameterRows, deferredDetailSection }) {
    ...
    function promptStatusMarkup(status, prefix = "") {
      const label = { snapshot: "由显示快照识别", summary: "组合或多阶段文本摘要", partial: "部分解析", missing: "未读取到文本" }[status];
    ...
    function comfyStageFields(stage) {
      ...
      for (const key of ["prompt_status", "negative_prompt_status"]) if (fields[key]) fields[key] = ({ exact: "直接文本", snapshot: "由显示快照识别", summary: "可读摘要，非等价条件", partial: "部分解析", missing: "未读取到文本" })[fields[key]] || fields[key];
      return fields;
    }
    ...
    function comfyDetailsMarkup(metadata, withCopy = false, lazy = false) {
      ...
      const hasSnapshot = stages.some((stage) => stage.prompt_sources?.length || stage.negative_prompt_sources?.length);
      ...
      return `<details class="comfy-workflow-info"><summary>采样阶段与条件 · ${stages.length} 个阶段</summary>${snapshotNote}<div class="comfy-output-list">${outputs}</div>${stageMarkup}${conditions}</details>`;
    }
