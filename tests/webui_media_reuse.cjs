/* Cross-view request budgets; use only the isolated tests/webui_harness.py API. */
...
async function viewerReady(inner, id) {
  await inner.waitForFunction((imageId) => {
    ...
    return viewer?.opener.isOpen && viewer.currSlide.data.image_id === imageId
      && viewer.currSlide.data.loadedDetail === "original" && image?.complete && image.naturalWidth > 1;
  }, id);
}
...
async function invalidDecodeRecovery(page, groupId, image, marker) {
  ...
  try {
    ...
    await inner.locator("#detailCopy:not(:disabled)").waitFor(); await started.promise;
    await openViewer(inner); await page.waitForTimeout(300); corruptResponse.release();
    await viewerReady(inner, image.id);
    assert.equal(attempts, 2, "a malformed HTTP-200 image must be decoded, evicted and fetched again by the viewer retry");
    const recovered = await inner.evaluate(({ item, broken }) => {
      const source = window.__reuseHooks.getImageMedia(item, "original");
      return !!source && source !== broken && source === window.__reuseViewer.currSlide.content.element.src;
    ...
    assert.equal(attempts, 2, "returning to detail must reuse the recovered original");
...
async function run(engine) {
  ...
  try {
    ...
    assert.equal(count("original", ids[0]), 1);
    const initialSequences = count("sequence"); assert.ok(initialSequences > 0);

    // The original is still pending in detail when the lightbox requests it.
    await openViewer(inner); await page.waitForTimeout(450);
    assert.equal(count("original", ids[0]), 1, "detail and lightbox must join the same pending original request");
    ...
    originalGate.release(); await viewerReady(inner, ids[0]);
    await inner.evaluate(() => window.__reuseViewer.goTo(2)); await viewerReady(inner, ids[2]);
    await frames(inner, 8);
    const beforeReturn = { preview: count("preview", ids[2]), original: count("original", ids[2]) };
    ...
    assert.equal(count("original", ids[2]), beforeReturn.original, "returning from the lightbox must reuse its selected original");

    await inner.locator("#closeDrawer").click();
    const beforeReopen = { preview: count("preview"), original: count("original"), metadata: count("metadata", ids[0]), sequence: count("sequence") };
    await inner.locator(`[data-gallery-id="${groupId}"] .gallery-info`).click();
    ...
    console.log(`${engine} 390: gallery preview reuse, preview-free metadata, pending original deduplication, lightbox return, reopen revalidation, browse/preview revision invalidation, HTTP/decode retry recovery and bounded media cache passed`);
  } finally {
