...
folder=Path(sys.argv[1]);name=sys.argv[2]
...
graph={
 ...
 "90":{"class_type":"easy showAnything","inputs":{"anything":["3",0],"text":snap_a}},
 ...
 "93":{"class_type":"easy showAnything","inputs":{"anything":["3",0],"text":snap_a}}
}
nodes=[];links=[]
for key,node in graph.items():
 ...
 widgets=[snap_b] if key=="90" else [snap_a] if key=="93" else []
 nodes.append({"id":int(key),"type":node["class_type"],"inputs":inputs,"widgets_values":widgets,"pos":[0,0],"size":[200,100]})
...
async function verify(browser, name, width) {
  ...
  try {
    ...
    assert.ok(a.observations.length >= 2); assert.ok(snapshots.every((candidate) => !["91", "92"].includes(candidate.node_id)));
    ...
    assert.match(candidateText, /关联显示快照/); assert.match(candidateText, /未验证是否为本次结果/); assert.match(candidateText, /API 执行图/); assert.match(candidateText, /界面工作流/); assert.match(candidateText, /同一输出存在不同快照/);
    assert.doesNotMatch(candidateText, /WRONG_OUTPUT_PORT|ALREADY_KNOWN_STATIC_OUTPUT/);
    ...
    assert.deepEqual(errors, []); console.log(`${name}: snapshots, provenance, conflicts, exact block replacement, user edits and deduplication passed`);
