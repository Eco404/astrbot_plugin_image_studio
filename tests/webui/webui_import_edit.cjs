/* Run only against a fresh tests/support/webui_harness.py instance, never a deployment. */
...
async function verify(browser, name, width) {
  ...
  try {
    ...
    // The root owns scroll locking; hiding body overflow breaks sticky headers.
    assert.equal(await frame.locator("html").evaluate((node) => getComputedStyle(node).overflow), "hidden");
    assert.equal(await frame.locator("body").evaluate((node) => getComputedStyle(node).overflow), "visible");
