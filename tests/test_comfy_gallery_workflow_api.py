...
from astrbot_plugin_image_studio.tests.test_comfy_api import PREFIX, setup, teardown
from astrbot_plugin_image_studio.tests.test_comfy_provider import config, graph
...
def test_gallery_rejects_cross_record_image_and_disabled_reuse(tmp_path, monkeypatch):
    async def run():
        ...
        try:
            ...
            original_header = app.state.plugin.store._generation_header
            ...
            monkeypatch.setattr(
                app.state.plugin.store, "_generation_header", restricted_header
            )
            response = await client.post(
                PREFIX + "comfy/import",
