...
from astrbot_plugin_image_studio.tests.backend.test_command_invocation import (
    CommandEvent,
)
from astrbot_plugin_image_studio.tests.backend.test_nai_batches import Executor
...
def test_partial_batch_reaches_caller_with_images_and_warning(tmp_path, source):
    async def run():
        ...
        if source == "webui":
            ...
            event = CommandEvent(f"/img mountain landscape --model {MODEL_REF} --n 3")
