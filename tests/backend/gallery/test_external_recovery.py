...
import pytest
from astrbot_plugin_image_studio.backend.gallery.errors import ExternalDeleteError
    ...
    async def run():
        ...
        try:
            ...
            if failure in {"permission", "invalid_path"}:
                ...
                monkeypatch.setattr(store.external_records, "external_path", reject)
                assert await store.gallery_image_file(result["image_id"]) is None
...
def test_bursts_and_active_scans_reuse_one_task_without_queued_followup(
    ...
    async def run():
        ...
        try:
            ...
            gate.set()
            await manager.close()
            assert store._context.external_issue_handler is None
            manager._notify_source_issue("nai", "missing")
            await store.close()
