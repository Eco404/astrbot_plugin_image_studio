...
from astrbot_plugin_image_studio.backend.parameters.exchange import export_parameters
from astrbot_plugin_image_studio.backend.models import ReferenceImage
from astrbot_plugin_image_studio.tests.backend.gallery.test_gallery_retention import (
    picture,
    record,
)
...
def test_single_image_context_preserves_exports_without_sibling_or_media_reads(
    ...
    async def run():
        ...
        projections = []
        asset_item = store.queries.asset_item

        def project(row, **kwargs):
            projections.append(row["id"])
            return asset_item(row, **kwargs)

        monkeypatch.setattr(store.queries, "asset_item", project)
...
def test_gallery_retention_cache_invalidates_and_never_controls_cleanup(
    ...
    async def run():
        ...
        calls = []
        calculate = store.maintenance.retention_status

        def counted(settings):
            calls.append(settings)
            return calculate(settings)

        monkeypatch.setattr(store.maintenance, "retention_status", counted)
        ...
        assert len(calls) == 7
        await asyncio.to_thread(store.maintenance.cleanup, history)
