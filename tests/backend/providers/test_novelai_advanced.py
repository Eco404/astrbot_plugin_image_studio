...
def test_vibe_cache_is_shared_for_same_account_but_isolates_inputs_model_and_endpoint():
    async def run():
        ...
        value = await executor._novelai._encode_vibe(configured, payload)
        assert (
            await executor._novelai._encode_vibe(
                replace(
        ...
        for different_provider, different_payload in (
            ...
            await executor._novelai._encode_vibe(different_provider, different_payload)
...
def test_concurrent_identical_vibes_encode_once_and_release_the_per_key_lock():
    async def run():
        ...
        tasks = [
            asyncio.create_task(
                executor._novelai._encode_vibe(
                    provider(model=V45, id=str(index)), payload
                )
            )
        ...
        assert len(session.encode_calls) == 1
        assert not executor._novelai._vibe_locks
...
def test_failed_vibe_encoding_never_sends_a_generation_request_or_caches_an_embedding(
    ...
    async def run():
        ...
        assert not executor._novelai._vibe_cache
        assert not executor._novelai._vibe_locks
...
def test_failed_encoding_is_not_repeated_by_other_batch_chunks(monkeypatch):
    async def run():
        ...
        current_time = [100.0]
        monkeypatch.setattr(
            "astrbot_plugin_image_studio.backend.providers.novelai.client.time.monotonic",
...
def test_second_vibe_failure_preserves_first_encoding_but_does_not_generate():
    ...
    async def run():
        ...
        assert len(executor._novelai._vibe_cache) == 1
        assert not executor._novelai._vibe_locks
...
def test_reusing_the_same_vibe_with_changed_strength_does_not_charge_a_second_encoding():
    async def run():
        ...
        assert SECRET not in json.dumps(list(executor._novelai._vibe_cache))
