...
from astrbot_plugin_image_studio.tests.backend.test_import_groups import image, stage
...
def test_append_rechecks_target_after_metadata_preparation(
    ...
    async def run():
        ...
        prepare_entries = store.imports.prepare_import_entries

        def changed_target(prepared):
            result = prepare_entries(prepared)
            if change == "deleted":
                store.records.delete_generation(target)
            else:
                with store._connect() as conn:
                    row = conn.execute(
        ...
        monkeypatch.setattr(store.imports, "prepare_import_entries", changed_target)
