...
from astrbot_plugin_image_studio.backend.api import generation as generation_api
...
from astrbot_plugin_image_studio.tests.backend.test_novelai_advanced import reference
from astrbot_plugin_image_studio.tests.backend.test_service_and_tool import ToolEvent
...
def test_webui_excess_ids_are_rejected_before_loading_staging(
    ...
    async def run():
        ...
        plugin.store.assets.load_staged_references = reject_read
        ...
        monkeypatch.setattr(generation_api, "web_request", SimpleNamespace(json=body))
